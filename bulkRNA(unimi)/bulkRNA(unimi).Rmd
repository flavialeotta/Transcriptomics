...
This report aims to show the results of BulkRNA transcriptomics analysis on tissues samples: specifically, it uses data from Gtex project (processed by Recount3), available at the following link http://159.149.160.56/GT_2022_GTEX/. The tissues chosen, whose .RDS files are downloaded in the current directory, are:
...
The analysis was performed on RStudio (v2025.09.0+387, R v4.4.2) using the following libraries:
...
For the analysis, I chose 3 different patients for each tissue. The choice depends on the quality of the data, which is evaluated with metrics:
...
```{r, echo=TRUE, message=FALSE, warning=FALSE, fig.height=12, fig.width=7}
df_rin <- data.frame(
...
combined_plot
```
From these plots, I decided to raise the RIN threshold to 6.5 and the percentage of uniquely mapped reads to 85%. The selected patients have the following characteristics:
